...
- 🎵 **Music Playback**: Full-featured audio player with play/pause, next/previous, and seek controls
- 📱 **Responsive Design**: Optimized for both desktop and mobile devices
- 🎨 **Theme Customization**: Multiple color themes with customizable primary, secondary, and tertiary colors
- 🔄 **Auto-Advance**: Automatically plays the next track when the current one ends
- 📱 **PWA Support**: Progressive Web App with install prompt and service worker
- 🔒 **iOS Compatibility**: Concatenated playlist support for iOS lock screen auto-advance
- 🎁 **Easter Eggs**: Support for secret tracks hidden in the playlist
- 📦 **Versioned Deployments**: Support for multiple versions of the app (v1, v2, etc.)
- 🎯 **Track Library**: Browse and select tracks from a visual library with album art
...
│   ├── MediaLibrary.tsx # Track library/browser
│   ├── ThemePicker.tsx  # Theme selection UI
│   ├── VersionPicker.tsx # Version selection
│   ├── PWAInstallPrompt.tsx # PWA installation prompt
│   └── DevBuildInfo.tsx # Development build info
├── contexts/
│   └── ThemeContext.tsx # Theme state management
├── hooks/
│   └── useTracks.ts     # Track data fetching and management

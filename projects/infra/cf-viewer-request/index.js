      ...
      res.on("end", () => {
        if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
          try {
            ...
            reject(
              new Error(
                "Failed to parse JSON response: " + e + ". Body=" + data
              )
            );
          }
          return;
...
function applyVersionRouting(request, cfg) {
  ...
  if (/^v\d+$/i.test(firstSegment)) {
    ...
    if (
      cfg.activeVersions.map((v) => v.toLowerCase()).includes(normalizedVersion)
    ) {
  ...
  try {
    ...
    if (
      ...
      (headers.authorization &&
        headers.authorization[0].value === cfg.authString)
    ) {
